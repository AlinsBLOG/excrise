...
Page({
  ...
  getData:function(){
    ...
    util.request(api.OrderDetail + user.checkLogin().id + "/detail/" + orderId).then(function (res) {
      if (res.success === true) {
        let data = res.data;
        data.prodGroups.map(function (item) {
  ...
  goToWechatPay:function(){
    ...
    util.request(api.ApplyAuthen + userId + "/applyAuthen/forCallApi", null, 'POST').then(function (res) {
      if (res.success === true) {
        ...
        util.request(res.data.baseUrl + '/platform-pay/prepare4Pay/' + res.data.appCode + '/applyForWxTinyAppPay?orderId=' + dataList.orderId + '&orderNo=' + dataList.orderNo + '&amount=' + dataList.totalAmount + '&openId=' + app.globalData.openId, null, 'POST').then(function (ressp) {
