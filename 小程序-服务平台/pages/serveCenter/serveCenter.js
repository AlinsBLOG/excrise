...
Page({
  ...
  userOrderList: function () {
    ...
    //加载指定用户订单列表
    util.request(api.QueryServeList + user.checkLogin().id + "/list?condCode=" + that.data.condCode + '&pageNo=' + that.data.pageNo + '&pageSize=' + that.data.pageSize).then(function (res) {
      console.log(res)
      if (res.success === true) {
        if (res.data) {
          let centerList = res.data;
          centerList.map(function (item) {
            item.countyName = user.getNameByCode(item.countyCode);
            return item;
          })
  ...
  queryOrderDetail: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/serveDetail/serveDetail?orderId=' + orderId,
    })
  },
